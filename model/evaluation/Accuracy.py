...
class BalancedPixelAccuracy(AbstractEvaluator):
    def __init__(self, threshold=0.5, mode="origin") -> None:
        super().__init__()
        self.name = "Balanced pixel-level Accuracy"
        self.desc = "Balanced Accuracy to counter class imbalance"
        self.threshold = threshold
        self.mode = mode    # 支持 origin / reverse / double 同原版
    ...
    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        ...
        if self.mode == "origin":
            ...
            raise RuntimeError(f"Balanced ACC no mode name {self.mode}")

        TPR = TP / (TP + FN + 1e-8)  # 正类 recall
        TNR = TN / (TN + FP + 1e-8)  # 负类 specificity
