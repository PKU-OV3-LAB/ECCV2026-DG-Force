...
class PixelBestIOU(AbstractEvaluator):
    """
    Pixel-level Best IoU (per-image), by sweeping thresholds in [0,1].
    Interface aligned with PixelIOU:
      - batch_update(predict, mask, shape_mask=None, *args, **kwargs) -> Tensor[B]
      - remain_update calls batch_update
      - epoch_update returns None
      - recovery returns None
    """
    def __init__(self, num_thresholds: int = 256, mode: str = "origin") -> None:
        super().__init__()
        self.name = "pixel-level Best IOU"
        self.desc = "pixel-level Best IOU"
        self.num_thresholds = int(num_thresholds)
        self.mode = mode  # "origin" | "reverse" | "double"

    def Cal_Best_IOU(self, predict, mask, shape_mask=None):
        """
        predict, mask: [B,1,H,W], predict in [0,1], mask in {0,1}
        returns: best_iou per-image, Tensor[B]
        """
        # Keep behavior consistent with your PixelIOU: multiply by shape_mask if provided
        if shape_mask is not None:
            predict = predict * shape_mask
            mask = mask * shape_mask

        B = predict.shape[0]
        # Flatten per image: [B, N]
        p = predict.float().flatten(1)
        y = mask.float().flatten(1)

        # Sweep thresholds
        T = self.num_thresholds
        thresholds = torch.linspace(0.0, 1.0, T, device=p.device).view(1, T, 1)  # [1,T,1]
        pred_bin = (p.unsqueeze(1) > thresholds).float()  # [B,T,N]

        # intersection & union per threshold
        inter = (pred_bin * y.unsqueeze(1)).sum(dim=-1)  # [B,T]
        union = pred_bin.sum(dim=-1) + y.unsqueeze(1).sum(dim=-1) - inter  # [B,T]

        iou = inter / (union + 1e-8)
        best_iou, _ = iou.max(dim=1)  # [B]
        return best_iou

        # ---------- (Optional stricter version: exclude invalid region instead of multiplying) ----------
        # if shape_mask is not None:
        #     v = (shape_mask.flatten(1) > 0).float()   # [B,N]
        # else:
        #     v = torch.ones_like(y)
        # pred_bin = pred_bin * v.unsqueeze(1)
        # yv = y * v
        # inter = (pred_bin * yv.unsqueeze(1)).sum(dim=-1)
        # union = pred_bin.sum(dim=-1) + yv.unsqueeze(1).sum(dim=-1) - inter
        # iou = inter / (union + 1e-8)
        # best_iou, _ = iou.max(dim=1)
        # return best_iou
    ...
    def batch_update(self, predict, mask, shape_mask=None, *args, **kwargs):
        ...
        if self.mode == "origin":
            return self.Cal_Best_IOU(predict, mask, shape_mask)

        elif self.mode == "reverse":
            return self.Cal_Best_IOU(1 - predict, mask, shape_mask)

        elif self.mode == "double":
            normal_iou = self.Cal_Best_IOU(predict, mask, shape_mask)
            reverse_iou = self.Cal_Best_IOU(1 - predict, mask, shape_mask)
            return torch.max(normal_iou, reverse_iou)

        else:
            raise RuntimeError(f"PixelBestIOU no mode name {self.mode}")
