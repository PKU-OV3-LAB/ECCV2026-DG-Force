...
from model.evaluation import PixelF1, PixelIOU, PixelAUC

from model.training_scripts.tester import test_one_epoch

def get_args_parser():
    parser = argparse.ArgumentParser('DG-Force testing launch!', add_help=True)
    # -------------------------------
    # Model name
...
def main(args, model_args):
    ...
    evaluator_list = [
        ...
        # DG-Force release keeps the standard release metrics only.
    ]
    ...
    if os.path.isfile(args.checkpoint_path):
        if not args.checkpoint_path.endswith(".pth"):
            raise ValueError(f"checkpoint_path must be a .pth file or a directory, got: {args.checkpoint_path}")
        chkpt_pair = [(0, os.path.basename(args.checkpoint_path))]
        ckpt_root = os.path.dirname(args.checkpoint_path) or "."
    else:
        chkpt_list = os.listdir(args.checkpoint_path)
        print(chkpt_list)
        ckpt_root = args.checkpoint_path
        chkpt_pair = []
        for chkpt in chkpt_list:
            ...
            if "-" in chkpt:
                try:
                    ...
                    epoch = 0
            chkpt_pair.append((epoch, chkpt))
        chkpt_pair.sort(key=lambda x: (x[0], x[1]))
    ...
    for epoch, chkpt_dir in chkpt_pair:
        if chkpt_dir.endswith(".pth"):
            ...
            ckpt = os.path.join(ckpt_root, chkpt_dir)
            ckpt = torch.load(ckpt, map_location="cpu", weights_only=False)
            model.module.load_state_dict(ckpt['model'])     
            
            # 存储各指标在所有数据集上的累积值
            avg_res = {type(eval).__name__:0.0 for eval in evaluator_list}
            avg_res_keys_list = list(avg_res.keys())
            ...
            for dataset_name, dataloader_test in dataset_dict.items():
                ...
                #　累积各个指标的值
                for idx, (k, v) in enumerate(test_stats.items()):
                    avg_res[avg_res_keys_list[idx]] += v
                    # print(f"[DEBUG] adding: {k} to {avg_res_keys_list[idx]} in avg_res")
            ...
            #　计算所有数据集上的平均值
            avg_res = {k:v/len(dataset_dict) for k,v in avg_res.items()}
            factor=10**4
            avg_res = {k:int(v*factor)/factor for k,v in avg_res.items()} # 结果保留4位小数
            print(f"Epoch[{epoch}] Average results of all datasets: {avg_res}\n")
