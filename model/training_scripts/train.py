...
from model.datasets import ManiDataset, JsonDataset, BalancedDataset
from model.transforms import RandomCopyMove, RandomInpainting

from model.evaluation import PixelF1

from model.training_scripts.tester import test_one_epoch
from model.training_scripts.trainer import train_one_epoch

def get_args_parser():
    parser = argparse.ArgumentParser('DG-Force training launch!', add_help=True)
...
def main(args, model_args):
    ...
    torch.use_deterministic_algorithms(True) # add
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.enabled = True # change
    ...
    if args.distributed:
        ...
        # model.register_comm_hook(state=None, hook=deterministic_fp64_allgather_sum) # 消除卡序带来的随机性
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # debug: test random seed for img list
        # print("epoch: ",epoch)
        # print("len loader: ", len(data_loader_train))
        # if epoch == 5: # for debug
        #     print("Debug break")
        #     return
        # else:
        #     log_path = f"output_dir_mesorch_test_random_seed/exp4_ep{epoch}.txt" # debug: test random seed for img list
        #     for data_dict in data_loader_train:
        #         with open(log_path,'a+') as f:
        #             for it in data_dict['image_path']:
        #                 print(it)
        #                 f.write(it+'\n')
        #     continue
